      ...
      <div className="grid gap-6 md:grid-cols-3">
        {steps.map((step, i) => (
          <Reveal key={step.title} delay={i * 100}>
            <div
              className={`relative h-full rounded-2xl border-2 ${step.border} bg-white p-7 shadow-pop-sm transition-transform duration-200 hover:-translate-y-1`}
            >
              <span
                className={`absolute -top-4 -left-3 flex h-9 w-9 items-center justify-center rounded-full border-2 ${step.border} bg-cream font-display text-sm font-semibold text-ink`}
              >
                {i + 1}
              </span>
              <div
